...
async function run() {
    console.log("OLD FORMAT TEST (Unwrapped + Uppercase Headers)...");
    ...
    // UNWRAPPED PAYLOAD (Old Style)
    const payload = JSON.stringify({
        title: "🚌 Bus 405 (OLD FORMAT REVERT)",
        body: "Testing if unwrapped style works better.",
    ...
    try {
        ...
        console.log("Sending with UPPERCASE headers...");
        ...
        if (resp.status !== 201) console.log(await resp.text());

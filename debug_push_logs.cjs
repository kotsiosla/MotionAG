...
async function checkPushLogs() {
    console.log("Fetching recent push notification attempts...");
    const { data, error } = await supabase
        .from('notifications_log')
        .select('*')
        .not('metadata->push_success', 'is', null)
    ...
    console.log(`Found ${data.length} push attempts:`);
    data.forEach(log => {

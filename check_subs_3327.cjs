...
async function check() {
    console.log("Checking subscriptions for Stop 3327...");
    ...
    const relevant = subs.filter(s =>
        ...
        s.stop_notifications.some(n => n.stopId === "3327" && n.enabled)
    );

    console.log(`Found ${relevant.length} active subscriptions for Stop 3327.`);
    relevant.forEach(s => {
        const setting = s.stop_notifications.find(n => n.stopId === "3327");

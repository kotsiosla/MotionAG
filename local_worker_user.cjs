...
async function sendPush(sub, payloadStr) {
    try {
        ...
        console.log(`[Push] Sent to ${sub.endpoint.slice(0, 20)}... Status: ${resp.status}`, text);
        return resp.ok;
...
async function check() {
    console.log(`[${new Date().toISOString()}] Checking Stop 2793...`);
    try {
        // 1. Get Subscription for Stop 2793
        // ...
        const subs = await fetchJson(`${SUPABASE_URL}/rest/v1/stop_notification_subscriptions?select=*&limit=50`);
        // ...
        // Find subs that have stopId "2793"
        const relevantSubs = subs.filter(s => s.stop_notifications && Array.isArray(s.stop_notifications) && s.stop_notifications.some(n => n.stopId === "2793" && n.enabled));

        if (relevantSubs.length === 0) {
            console.log("No active subscription found for stop 2793.");
            return;
        }
        console.log(`Found ${relevantSubs.length} subscriptions for Stop 2793`);

        // 2. Fetch Arrivals for Stop 2793
        const arrivalsUrl = `${SUPABASE_URL}/functions/v1/gtfs-proxy/arrivals?stopId=2793`;
        const arrivalsResp = await fetchJson(arrivalsUrl);
        const arrivals = arrivalsResp.data || [];

        console.log(`Arrivals found: ${arrivals.length}`);

        for (const targetSub of relevantSubs) {
            const setting = targetSub.stop_notifications.find(n => n.stopId === "2793");
            // FORCE TEST: Ignore user setting, fire for anything < 99 mins
            const threshold = 99;
            console.log(`   - Sub ${targetSub.id.slice(0, 5)} Threshold: ${threshold}m (FORCED)`);
            ...
            for (const arrival of arrivals) {
                ...
                if (mins <= threshold) {
                    ...
                    const payload = JSON.stringify({
                        ...
                        data: { url: `https://kotsiosla.github.io/MotionAG/?stop=2793` }
                    });

                    // UPDATED SEND PUSH LOGIC INLINE FOR CERTAINTY
                    const sub = targetSub;
...
// Loop
console.log("Starting Local Worker for User (Stop 2793)...");
check();
// Run every 10s
setInterval(check, POLL_INTERVAL);
// // setInterval(check, POLL_INTERVAL);
// // Just run once and exit to capture log
// check().then(() => setTimeout(() => process.exit(0), 2000));

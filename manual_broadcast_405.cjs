...
// CONFIG (From test-push/index.ts)
...
async function run() {
    ...
    const payload = JSON.stringify({
        notification: {
            title: "🚌 Bus 405 (MANUAL RE-RUN)",
            body: "Replicating the validation test from last night (23:17).",
    ...
    for (const sub of subscriptions) {
        try {
            console.log(`Sending to ${sub.endpoint.slice(0, 30)}...`);
            ...
            console.error("Error sending to " + sub.endpoint.slice(0, 10), e.message);
        }
    }
}

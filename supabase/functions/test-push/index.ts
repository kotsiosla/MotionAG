...
serve(async (req) => {
  ...
  try {
    ...
    const FALLBACK_VAPID_PUBLIC_KEY = 'BI8fqvzilk9dOm-qzC13UJzDRE9ozlgKqkNcOmHkRHFZ_j8UlCNhRuc_ED2JzTF7leQErAHNGizfRCCLtNz0aDE';
    const FALLBACK_VAPID_PRIVATE_KEY = 'UnuF9N7n9-A0qIdXoCFXKtRfcTw3CBVF19UZfilLP-c';
    ...
    // Allow overriding icon (pass explicit null to remove it, or string to change it)
    const icon = requestBody.icon !== undefined ? requestBody.icon : '/MotionAG/pwa-192x192.png';
    console.log(`[test-push:${requestId}] Notification title: "${title}", body: "${body}", icon: "${icon}"`);
    ...
    const payloadObj: any = {
      title,
      body,
      url: 'https://kotsiosla.github.io/MotionAG/', // Correct absolute base path
    };
    ...
    for (const sub of subscriptions) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log(`[test-push:${requestId}] ERROR_DETAILS_START`);
          const responseHeadersObj = Object.fromEntries(response.headers.entries());
          console.log(`[test-push:${requestId}] ERROR_HEADERS: ${JSON.stringify(responseHeadersObj)}`);
          console.log(`[test-push:${requestId}] ERROR_BODY_LENGTH: ${responseText.length}`);
          console.log(`[test-push:${requestId}] ERROR_BODY_CONTENT: ${responseText}`);
          console.log(`[test-push:${requestId}] ERROR_DETAILS_END`);

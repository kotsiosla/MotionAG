  ...
  window.onerror = async function (message, source, lineno, colno, error) {
    try {
      ...
      await sb.from('notifications_log').insert({
        ...
        alert_level: 0,
        metadata: { message, source, lineno, colno, error: String(error), version: 'v1.5.17', timestamp: new Date().toISOString() }
      });
    } catch { }
  };
  ...
  (async () => {
    try {
      ...
      await sb.from('notifications_log').insert({
        ...
        route_id: 'APP_BOOT',
        alert_level: 0,
        metadata: { step: 'BOOTSTRAP', version: 'v1.5.17', href: window.location.href, timestamp: new Date().toISOString() }
      });
    } catch { }
  })();
}

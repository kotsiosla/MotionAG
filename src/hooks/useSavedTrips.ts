...
export function generateCalendarUrl(trip: SavedTrip): string {
  ...
  trip.journey.legs.forEach((leg, idx) => {
    if (leg.type === 'walk') {
      descriptionLines.push(`${idx + 1}. 🚶 Περπάτημα ${Number((leg.walkingMinutes || 0).toFixed(2))} λεπτά (${leg.walkingMeters}μ)`);
...
export function formatTripForShare(trip: SavedTrip): string {
  ...
  trip.journey.legs.forEach((leg, idx) => {
    if (leg.type === 'walk') {
      text += `${idx + 1}. 🚶 Περπάτημα ${Number((leg.walkingMinutes || 0).toFixed(2))}'\n`;
    } else if (leg.type === 'bus') {

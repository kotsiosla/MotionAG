...
async function run() {
    console.log("Fetching TOP 3 Recently UPDATED subscriptions...");

    // Fetch from both tables, sort by updated_at (or created_at if updated_at null)
    const { data: stopSubs } = await supabase.from('stop_notification_subscriptions').select('*').order('updated_at', { ascending: false }).limit(3);
    const { data: pushSubs } = await supabase.from('push_subscriptions').select('*').order('created_at', { ascending: false }).limit(3); // push_subs might not have updated_at

    let all = [...(stopSubs || []), ...(pushSubs || [])];

    // Sort all by updated_at or created_at desc
    all.sort((a, b) => {
    ...
    console.log(`Found ${top3.length} targets.`);

    const payload = JSON.stringify({
        notification: {
            title: "🚌 TARGETED TEST (Updated Sub)",
            body: "Testing the most recently synced connection.",
            ...
            data: {
                url: "https://kotsiosla.github.io/MotionAG/?test=true",
                logKey: ANON_KEY
            }
        }
    });
    ...
    for (const sub of top3) {
        try {
            const time = sub.updated_at || sub.created_at;
            console.log(`Sending to SUB (${time}) - Endpoint: ...${sub.endpoint.slice(-10)}`);

            const aud = new URL(sub.endpoint).origin;
            const jwt = await createVapidJwt(aud, 'mailto:info@motionbus.cy', VAPID_PUB, VAPID_PRIV);
            const { ciphertext, salt, localPub } = await encryptPayload(payload, sub.p256dh, sub.auth);
